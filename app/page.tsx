...
const markets = ["Nigeria", "Ghana", "Côte d’Ivoire", "Senegal", "Benin", "Cameroon"];
const reasons = [
  ["24/7", "Operations Coordination", "Responsive planning and support for time-sensitive assignments."],
  ["01", "Single Point of Contact", "Clear communication from initial briefing through mission completion."],
  ["INTL", "International Standards", "Professional service delivery built for global corporate expectations."],
  ["LOCAL", "Regional Knowledge", "Local coordination informed by operational realities across West Africa."],
];
    ...
    <main>
      ...
      <section className="hero heroV2" id="top">
        ...
        <div className="heroContent">
          ...
          <h1>Securing Africa.<br /><span>Protecting global business.</span></h1>
          ...
          <div className="actions"><a className="button primary" href="/contact">Request Protection</a><a className="button secondary" href="mailto:info@security-solutions.africa">Talk to an Expert</a></div>
        ...
        <aside className="heroGlassPanel"><span className="panelNumber">01</span><p>Executive protection and secure mobility for high-value operations.</p><a href="/executive-protection">Explore capability →</a></aside>
      </section>
      ...
      <section className="reasonsSection"><div className="sectionHeading"><p className="eyebrow">Why international clients choose us</p><h2>Operational confidence, from briefing to completion.</h2></div><div className="reasonsGrid">{reasons.map(([code,title,text])=><article key={title}><strong>{code}</strong><h3>{title}</h3><p>{text}</p></article>)}</div></section>

      <section className="industriesSection" id="industries"><div className="sectionHeading"><p className="eyebrow">Industry support</p><h2>Built for demanding African operations.</h2></div><div className="industryGrid">{industries.map((industry,index)=><article key={industry}><span>0{index+1}</span><h3>{industry}</h3></article>)}</div></section>

      <section className="coverageV2" id="coverage"><div className="africaMap"><div className="mapAura" /><span className="mapTitle">West Africa</span>{markets.map((market,index)=><span className={`mapPoint point${index+1}`} key={market}><i />{market}</span>)}</div><div className="coverageCopy"><p className="eyebrow">Regional capability</p><h2>Starting in Nigeria. Built for Africa.</h2><p>Our initial operating focus is Nigeria, supported by a scalable regional coordination model for selected assignments across West Africa.</p><div className="marketList">{markets.map((market)=><span key={market}>{market}</span>)}</div><a className="button primary" href="/contact">Plan an operation</a></div></section>

      <section className="darkSection" id="process"><div className="sectionHeading"><p className="eyebrow">How it works</p><h2>Clear planning. Controlled delivery.</h2></div><div className="processGrid">{[["01","Briefing","Share the location, timing, traveller profile and required support."],["02","Assessment","We review routes, local conditions and operational constraints."],["03","Planning","A tailored security and movement plan is prepared for approval."],["04","Deployment","Resources are coordinated and supported throughout the assignment."]].map(([number,title,text])=><article key={number}><span>{number}</span><h3>{title}</h3><p>{text}</p></article>)}</div></section>
