...
async function savePortalRequest(record:Record<string,string>){
  ...
  const response=await fetch(`${url}/rest/v1/client_requests`,{
    method:"POST",
  ...
  if(!response.ok){
    console.error("Client portal database error:",await response.text());
    return false;
  }
  return true;
}
...
export async function POST(request:Request){try{
  ...
  const startedAt=Number(body.startedAt||0),elapsed=Date.now()-startedAt;
  if(!startedAt||elapsed<2000||elapsed>86400000)return NextResponse.json({message:"Please refresh the page and submit the form again."},{status:400});
  ...
  const saved=await savePortalRequest({reference,email,name,company,phone,request_type:requestType,urgency,service,country,location,required_date:requiredDate,people,details,status:"Received",next_step:"Our operations team will review the request and contact you with the next steps."});
  ...
  if(apiKey){
    ...
    try{
      await sendEmail(apiKey,{from,to:["info@security-solutions.africa"],reply_to:email,subject:`[${urgency}] ${reference} — ${service} — ${name}`,html:internalHtml});
    ...
    try{
      await sendEmail(apiKey,{from,to:[email],reply_to:"info@security-solutions.africa",subject:`Request received — ${reference}`,html:clientHtml});
    }catch(error){
      console.error("Resend client confirmation error:",error);
    }
  }

  if(!saved&&!internalEmailSent){
    return NextResponse.json({message:"The request system is not fully configured. Please email info@security-solutions.africa directly."},{status:503});
  }

  return NextResponse.json({
    message:saved
      ? "Your request has been recorded for operational review."
      : "Your request has been delivered to our operations team.",
    reference,
    emailConfirmationAvailable:Boolean(apiKey),
  });
}catch(error){
