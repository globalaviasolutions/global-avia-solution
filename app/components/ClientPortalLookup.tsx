...
export default function ClientPortalLookup(){
  ...
  return <div className="portalShell">
    ...
    {record&&<section className="portalDashboard portalDashboardV2" aria-live="polite">
      ...
      {tab==="overview"&&<div className="portalTabPanel"><div className="portalStatusSummary"><span>Current stage</span><h3>{record.status}</h3><p>{stageCopy[record.status]||"Your request is being managed according to the latest confirmed status."}</p></div><div className="portalTimeline">{stages.map((s,i)=><div className={i<=active?"complete":""} key={s}><i/><span>{s}</span></div>)}</div><div className="portalCards"><article><span>Service</span><strong>{record.service}</strong></article><article><span>Urgency</span><strong>{record.urgency}</strong></article><article><span>Location</span><strong>{record.location}, {record.country}</strong></article><article><span>Required date</span><strong>{record.requiredDate||"To be confirmed"}</strong></article><article><span>Personnel</span><strong>{record.people||"Not specified"}</strong></article><article><span>Submitted</span><strong>{new Date(record.createdAt).toLocaleDateString("en-GB")}</strong></article></div><div className="portalNext"><p className="eyebrow">Next step</p><h3>{record.nextStep}</h3>{record.notes&&<p>{record.notes}</p>}<a href={mailHref}>Contact operations about this request →</a></div>
      <div className="portalHistory"><p className="eyebrow">Request history</p><h3>Recorded updates</h3>{record.history?.length?<div>{record.history.map((event,index)=><article key={`${event.created_at}-${index}`}><i/><div><span>{new Date(event.created_at).toLocaleString("en-GB",{dateStyle:"medium",timeStyle:"short"})}</span><strong>{event.event_type}: {event.status}</strong>{event.next_step&&<p>{event.next_step}</p>}{event.client_note&&<small>{event.client_note}</small>}</div></article>)}</div>:<p className="portalHistoryEmpty">No additional updates have been recorded yet.</p>}</div></div>}
