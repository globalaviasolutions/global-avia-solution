...
export default function ClientPortalLookup(){
  ...
  return <div className="portalShell">
    <form className="portalLogin" onSubmit={submit}>
      ...
      <label><span>Reference number</span><input name="reference" value={reference} onChange={e=>setReference(e.target.value.toUpperCase())} placeholder="ASS-20260802-A1B2C3" required maxLength={40} autoComplete="off"/></label>
      <label><span>Email used in the request</span><input name="email" value={email} onChange={e=>setEmail(e.target.value)} type="email" required maxLength={160} autoComplete="email"/></label>
      <button className="button primary" disabled={loading}>{loading?"Checking…":"View request dashboard"}</button>
      <p className="portalPrivacy">Both fields must match the original request. The portal deliberately excludes sensitive operational instructions, routes and personal identity documents.</p>
    ...
    {record&&<section className="portalDashboard portalDashboardV2" aria-live="polite">
      ...
      <nav className="portalTabs" aria-label="Request dashboard sections">
        {(["overview","messages","documents","invoices"] as PortalTab[]).map(item=><button type="button" key={item} className={tab===item?"active":""} onClick={()=>setTab(item)}>{item[0].toUpperCase()+item.slice(1)}</button>)}
      </nav>
      ...
      {tab==="overview"&&<div className="portalTabPanel">
        ...
        <div className="portalCards">
          ...
          <article><span>Personnel</span><strong>{record.people||"Not specified"}</strong></article><article><span>Submitted</span><strong>{new Date(record.createdAt).toLocaleDateString("en-GB")}</strong></article>
        </div>
